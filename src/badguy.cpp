      ...
      if(mode == KICK && changed != dir)
        {
          ...
          if (base.x < scroll_x + screenW()/2 - 10)
            play_sound(sounds[SND_RICOCHET], SOUND_LEFT_SPEAKER);
          else if (base.x > scroll_x + screenW()/2 + 10)
...
BadGuy::action(double frame_ratio)
{
  ...
  if (base.x <= scroll_x + screenW() + OFFSCREEN_DISTANCE)
    seen = true;
...
BadGuy::draw()
{
  ...
  if(base.x <= scroll_x - base.width || base.x >= scroll_x + screenW())
    return;
