  ...
  if (old_x_pos != -1)
    {
      ...
      for(std::vector<ResetPoint>::iterator i = get_level()->reset_points.begin();
        ...
        {
          if (i->x - screenW()/2 < old_x_pos && best_reset_point.x < i->x)
            best_reset_point = *i;
        }
      ...
      if (best_reset_point.x != -1)
        {
          ...
          scroll_x = best_reset_point.x - screenW()/2;
        }
    }
      ...
      while (SDL_PollEvent(&event))
        {
          ...
          switch(event.type)
            ...
            case SDL_JOYBUTTONDOWN:
              if (event.jbutton.button == 11) // minus
                on_escape_press();
              break;
            }
        }
			 ...
			 tux.input.down = UP;
                ...
                case SDL_JOYBUTTONDOWN:
                  if (event.jbutton.button == 12) tux.input.left = DOWN; // dpad left
                    ...
                    tux.input.up = DOWN;
                  else if (event.jbutton.button == joystick_keymap.x_button || event.jbutton.button == joystick_keymap.y_button)
                    tux.input.fire = DOWN;
  ...
  if(game_pause)
    {
      ...
      for(int i = 0; i < x; ++i)
        {
          fillrect(i % 2 ? (pause_menu_frame * i)%screenW() : -((pause_menu_frame * i)%screenW()) ,(i*20+pause_menu_frame)%screenH(),screenW(),10,20,20,20, rand() % 20 + 1);
        }
      fillrect(0,0,screenW(),screenH(),rand() % 50, rand() % 50, rand() % 50, 128);
      blue_text->drawf("PAUSE - Press '+' To Play", 0, 230, A_HMIDDLE, A_TOP, 1);
    }
...
GameSession::drawstatus()
{
  ...
  white_text->draw("COINS", screenH(), 0, 1);
  ...
  if(show_fps)
    {
      ...
      white_text->draw("FPS", screenH(), 40, 1);
      gold_text->draw(str, screenH() + 60, 40, 1);
    }
}
