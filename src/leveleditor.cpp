...
int leveleditor(char* filename)
{
  ...
  while(true)
  {
    ...
    if(Menu::current() == select_tilegroup_menu)
    {
      ...
      {
        select_tilegroup_menu->set_pos(screenW() - 64 + select_tilegroup_menu_effect.get_left(),
                                       66,-0.5,0.5);
      }
      else
        select_tilegroup_menu->set_pos(screenW() - 64,66,-0.5,0.5);
    }
    else if(Menu::current() == select_objects_menu)
    {
      if(select_objects_menu_effect.check())
      {
        select_objects_menu->set_pos(screenW() - 64 + select_objects_menu_effect.get_left(),82,-0.5,0.5);
      }
      else
        select_objects_menu->set_pos(screenW() - 64,82,-0.5,0.5);
    }
    ...
    if(le_world != NULL)
    {
      ...
      if(pos_x < 0)
        pos_x = 0;
      if(pos_x > (le_world->get_level()->width * 32 + 2*32) - screenW())
        pos_x = (le_world->get_level()->width * 32 +2*32) - screenW();
    ...
    if (now_time < last_time + FPS) {} // dude no don't delay what's wrong with you
...
void le_init_menus()
{
  ...
  for(std::set<TileGroup>::iterator it = tilegroups->begin();
        it != tilegroups->end(); ++it )
    {
      ...
      tileid++;
      tilegroups_map[(*it).name] = new ButtonPanel(screenW() - 64,96, 64, 318);
      i = 0;
  ...
  objects_map["BadGuys"] = new ButtonPanel(screenW() - 64,96, 64, 318);
...
int le_init()
{
  ...
  /* Load buttons */
  le_save_level_bt = new Button("/images/icons/save.png","Save level", SDLK_F6,screenW()-64,32);
  le_exit_bt = new Button("/images/icons/exit.png","Exit", SDLK_F10,screenW()-32,32);
  le_next_level_bt = new Button("/images/icons/up.png","Next level", SDLK_PAGEUP,screenW()-64,0);
  le_previous_level_bt = new Button("/images/icons/down.png","Previous level",SDLK_PAGEDOWN,screenW()-32,0);
  le_rubber_bt = new Button("/images/icons/rubber.png","Rubber",SDLK_DELETE,screenW()-32,48);
  le_select_mode_one_bt = new Button ("/images/icons/select-mode1.png","Select single tile",SDLK_F3,screenW()-64,48);
  le_select_mode_two_bt = new Button("/images/icons/select-mode2.png","Select multiple tiles",SDLK_F3,screenW()-64,48);
  le_test_level_bt = new Button("/images/icons/test-level.png","Test level",SDLK_F4,screenW()-64,screenH() - 64);
  le_settings_bt = new Button("/images/icons/settings.png","Level settings",SDLK_F5,screenW()-32,screenH() - 64);
  le_move_left_bt = new Button("/images/icons/left.png","Move left",SDLK_LEFT,0,0);
  le_move_right_bt = new Button("/images/icons/right.png","Move right",SDLK_RIGHT,screenW()-80,0);
  le_tilegroup_bt = new Button("/images/icons/tilegroup.png","Select Tilegroup", SDLK_F7,screenW()-64,64);
  le_objects_bt = new Button("/images/icons/objects.png","Select Objects", SDLK_F8,screenW()-64,80);
  le_object_select_bt = new Button("/images/icons/select-one.png","Select an Object", SDLK_s, screenW() - 64, screenH()-98);
  le_object_properties_bt = new Button("/images/icons/properties.png","Edit object properties", SDLK_p, screenW() - 32, screenH()-98);
  ...
  le_tilemap_panel = new ButtonPanel(screenW()-64,screenH()-32,32,32);
...
void le_drawminimap()
{
  ...
  int mini_tile_width;
  if(screenW() - 64 > le_world->get_level()->width * 4)
    mini_tile_width = 4;
  else if(screenW() - 64 > le_world->get_level()->width * 2)
    mini_tile_width = 2;
  else
    mini_tile_width = 1;
  int left_offset = (screenW() - 64 - le_world->get_level()->width*mini_tile_width) / 2;
...
void le_drawinterface()
{
  ...
  if(le_world != NULL)
  {
    ...
    {
      for(x = 0; x < 19; x++)
        fillrect(x*32 - ((int)pos_x % 32), 0, 1, screenH(), 225, 225, 225,255);
      for(y = 0; y < 15; y++)
        fillrect(0, y*32, screenW() - 32, 1, 225, 225, 225,255);
    }
  }
  ...
  /* draw button bar */
  fillrect(screenW() - 64, 0, 64, screenH(), 50, 50, 50,255);
...
void le_drawlevel()
{
  ...
  {
    s = (int)((float)pos_x * ((float)le_world->get_level()->bkgd_speed/100.0f)) % screenW();
    le_world->get_level()->img_bkgd->draw_part(s,0,0,0,
        le_world->get_level()->img_bkgd->w - s - 32, le_world->get_level()->img_bkgd->h);
    le_world->get_level()->img_bkgd->draw_part(0,0,screenW() - s - 32 ,0,s,
...
void le_change_object_properties(GameObject *pobj)
{
  ...
  if(pobj->type() == "BadGuy")
  {
    BadGuy* pbad = static_cast<BadGuy*>(pobj);
...
void le_checkevents()
{
  ...
  while(SDL_PollEvent(&event))
  {
    ...
    else
    {
      ...
      if(event.type == SDL_KEYDOWN
          || ((event.type == SDL_MOUSEBUTTONDOWN || SDL_MOUSEMOTION)
              && (event.motion.x > 0
                  && event.motion.x < screenW() - 64 &&
                  event.motion.y > 0 && event.motion.y < screenH())))
      {
        switch(event.type)
        {
    ...
    if(le_world != NULL)
    {
      if(event.type == SDL_KEYDOWN || event.type == SDL_KEYUP || ((event.type == SDL_MOUSEBUTTONDOWN || SDL_MOUSEMOTION) && (event.motion.x > screenW()-64 && event.motion.x < screenW() &&
          event.motion.y > 0 && event.motion.y < screenH())))
      {
        le_mouse_pressed[LEFT] = false;
        le_mouse_pressed[RIGHT] = false;
        ...
        if(!Menu::current())
        {
          ...
          case BUTTON_CLICKED:
            ...
            select_tilegroup_menu->set_pos(screenW() - 64,100,-0.5,0.5);
            break;
          case BUTTON_WHEELUP:
            if(cur_tilegroup.empty())
            {
          ...
          case BUTTON_CLICKED:
            ...
            select_objects_menu->set_pos(screenW() - 64,100,-0.5,0.5);
            break;
          case BUTTON_WHEELUP:
	    ...
	    else if(le_current.IsObject())
	    {
          ...
          else
          {
            if(le_current.IsObject())
            {
              ...
              if(type == "BadGuy")
              {
                BadGuy* pbadguy = static_cast<BadGuy*>(le_current.obj);
