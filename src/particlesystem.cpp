...
{
    virtual_width = screenW();
    virtual_height = screenH();
}
...
void ParticleSystem::draw(float scrollx, float scrolly, int layer)
{
    ...
    for(i = particles.begin(); i != particles.end(); ++i) {
        ...
        // particle on screen
        if(x >= screenW() && xmax >= screenW())
            continue;
        if(y >= screenH() && ymax >= screenH())
            continue;
        
        if(x > screenW()) x -= virtual_width;
        if(y > screenH()) y -= virtual_height;
