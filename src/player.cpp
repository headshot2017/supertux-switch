...
bool Player::is_dead()
{
  if(base.y > screenH() || base.x < scroll_x - AUTOSCROLL_DEAD_INTERVAL)  // last condition can happen in auto-scrolling
    return true;
  else
    return false;
}
...
Player::check_bounds(bool back_scrolling, bool hor_autoscroll)
{
  ...
  if(hor_autoscroll)
    ...
    if(base.x + base.width > scroll_x + screenW())
      base.x = scroll_x + screenW() - base.width;
    }
