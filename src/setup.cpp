...
#define DATA_PREFIX "/switch/supertux-switch/data"
...
{
  char *home = "/switch/supertux-switch/home";
  char str[1024];
...
void st_menu(void)
{
  ...
  main_menu->set_pos(screenW()/2, 335);
...
void st_video_setup_sdl(void)
{
  ...
  if (use_fullscreen)
    {
      window = SDL_CreateWindow("SuperTux " VERSION, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_W, SCREEN_H, SDL_WINDOW_FULLSCREEN);
      if (window == NULL)
        {
		  char err[256];
		  sprintf(err, "Warning: I could not set up fullscreen video for "
                  "640x480 mode.\n"
                  "The Simple DirectMedia error that occured was:\n"
                  "%s\n\n", SDL_GetError());
		  SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_WARNING, "Warning", err, window);
    ...
    {
      window = SDL_CreateWindow("SuperTux " VERSION, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_W, SCREEN_H, 0);
...
void st_video_setup_gl(void)
{
  ...
  if (use_fullscreen)
    {
      window = SDL_CreateWindow("SuperTux " VERSION, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_W, SCREEN_H, SDL_WINDOW_FULLSCREEN | SDL_WINDOW_OPENGL);
      if (window == NULL)
        {
		  char err[256];
		  sprintf(err, "Warning: I could not set up fullscreen video for "
    ...
    {
      window = SDL_CreateWindow("SuperTux " VERSION, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_W, SCREEN_H, SDL_WINDOW_OPENGL);
  ...
  glLoadIdentity();
  glOrtho(0, screenW(), screenH(), 0, -1.0, 1.0);
...
void st_audio_setup(void)
{
      ...
      if (SDL_Init(SDL_INIT_AUDIO) < 0)
        {
          ...
          if (use_sound || use_music)
            {
			  char err[256];
			  sprintf(err, "Warning: I could not initialize audio!\n"
                      "The Simple DirectMedia error that occured was:\n"
                      "%s\n\n", SDL_GetError());
			  SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_WARNING, "Warning", err, window);
  ...
  if (audio_device)
    {
      if (open_audio(48000, AUDIO_S16, 2, 2048) < 0)
        {
          /* only print out message if sound or music
             was not disabled at command-line
           */
