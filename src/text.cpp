  ...
  if(text != NULL)
    {
      if(halign == A_RIGHT)  /* FIXME: this doesn't work correctly for strings with newlines.*/
        x += screenW() - (strlen(text)*w);
      else if(halign == A_HMIDDLE)
        x += screenW()/2 - ((strlen(text)*w)/2);
...
{
  erasetext(text, screenW() / 2 - (strlen(text) * 8), y, ptexture, update, shadowsize);
}
  ...
  while(done == 0)
    {
      ...
      while(SDL_PollEvent(&event))
        switch(event.type)
		  ...
		  case SDL_JOYAXISMOTION:
			...
			{
				if (event.jaxis.value > 512)
					speed += SPEED_INC;
				else if (event.jaxis.value < -512)
					speed -= SPEED_INC;
			}
			break;
      ...
      for(int i = 0; i < length; i++)
        ...
        switch(names.item[i][0])
          {
          case ' ':
            white_small_text->drawf(names.item[i]+1, 0, screenH()+y-int(scroll),
                A_HMIDDLE, A_TOP, 1);
            y += white_small_text->h+ITEMS_SPACE;
            break;
          case '	':
            white_text->drawf(names.item[i]+1, 0, screenH()+y-int(scroll),
                A_HMIDDLE, A_TOP, 1);
            y += white_text->h+ITEMS_SPACE;
            break;
          case '-':
            white_big_text->drawf(names.item[i]+1, 0, screenH()+y-int(scroll),
                A_HMIDDLE, A_TOP, 3);
            y += white_big_text->h+ITEMS_SPACE;
            break;
          default:
            blue_text->drawf(names.item[i], 0, screenH()+y-int(scroll),
                A_HMIDDLE, A_TOP, 1);
      ...
      if(screenH()+y-scroll < 0 && 20+screenH()+y-scroll < 0)
        done = 1;
