...
World::draw()
{
  ...
  if(level->img_bkgd)
    {
      int s = (int)((float)scroll_x * ((float)level->bkgd_speed/100.0f)) % screenW();
      level->img_bkgd->draw_part(s, 0,0,0,level->img_bkgd->w - s, level->img_bkgd->h);
      level->img_bkgd->draw_part(0, 0,screenW() - s ,0,s,level->img_bkgd->h);
    }
  else
    {
...
void World::scrolling(double frame_ratio)
{
  ...
  if (level->back_scrolling || debug_mode)
  {
    ...
    if(scrolling_timer.check())
    {
      ...
      if (tux.physic.get_velocity_x() > 0)
        final_scroll_x = tux_pos_x - (screenW() - X_SPACE);
    ...
    {
      if (tux.physic.get_velocity_x() > 0 && scroll_x < tux_pos_x - (screenW() - X_SPACE))
        scroll_x = tux_pos_x - (screenW() - X_SPACE);
      else if (tux.physic.get_velocity_x() < 0 && scroll_x > tux_pos_x - X_SPACE && level->back_scrolling)
        scroll_x = tux_pos_x - X_SPACE;
      else
      {
        if (tux.dir == RIGHT && scroll_x < tux_pos_x - (screenW() - X_SPACE))
            scroll_x = tux_pos_x - (screenW() - X_SPACE);
  ...
  if(scroll_x < 0)
    scroll_x = 0;
  if(scroll_x > level->width * 32 - screenW())
    scroll_x = level->width * 32 - screenW();
}
