      ...
      for(Levels::iterator i = levels.begin(); i != levels.end(); ++i)
        {
          if (i->x == tux->get_tile_pos().x && 
              i->y == tux->get_tile_pos().y)
            {
              if(!i->name.empty())
                {
              white_text->draw_align(i->title.c_str(), screenW()/2, screenH(),  A_HMIDDLE, A_BOTTOM);
                }
				  else if (i->teleport_dest_x != -1) {
				  	if(!i->teleport_message.empty())
               	 gold_text->draw_align(i->teleport_message.c_str(), screenW()/2, screenH(),  A_HMIDDLE, A_BOTTOM);
				  }
              ...
              if(!i->display_map_message.empty() && !i->passive_message)
                gold_text->draw_align(i->display_map_message.c_str(),
                     screenW()/2, screenH() - 30,A_HMIDDLE, A_BOTTOM);
              break;
            }
        }
    }
  ...
  while(!quit)
    {
      ...
      if (1)
        {
          offset.x = -tux_pos.x + screenW()/2;
          offset.y = -tux_pos.y + screenH()/2;

          if (offset.x > 0) offset.x = 0;
          if (offset.y > 0) offset.y = 0;

          if (offset.x < screenW() - width*32) offset.x = screenW() - width*32;
          if (offset.y < screenH() - height*32) offset.y = screenH() - height*32;
        } 
